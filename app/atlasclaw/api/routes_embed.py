...
class EmbedBootstrapRequest(StrictRequest):
    """Validate one AtlasClaw surface against a configured embed profile."""

    integration_id: str = Field(min_length=1, max_length=128)
    surface: Literal["floating", "menu"]
    host_origin: Optional[str] = Field(default=None, max_length=2048)
...
def _integration_or_404(ctx: APIContext, integration_id: str) -> Any:
    registry = ctx.embed_integration_registry
    integration = registry.get(integration_id) if registry is not None else None
...
def register_embed_routes(router: APIRouter) -> None:
    ...
    async def bootstrap_embed(
        ...
        integration = _integration_or_404(ctx, request.integration_id)
        if request.surface == "floating":
            if request.host_origin not in integration.config.allowed_origins:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Host origin is not allowed",
                )
        ...
        elif request.host_origin is not None or request.nonce is not None:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="Menu bootstrap does not accept Host origin or nonce",
            )
        active_session_key = await _validated_chat_active_session(
            ...
            session_scope=integration.config.session_scope,
        )
    ...
    async def resolve_embed_context(
        ...
        generation_registered = ctx.embed_context_store.mark_latest(
            ...
            state_ttl_seconds=integration.config.context_ttl_seconds,
        )
