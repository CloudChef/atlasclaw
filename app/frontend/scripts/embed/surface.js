 ...
 * @returns {{embedded: boolean, surface: string|null, integrationId: string|null, hostOrigin: string|null, nonce: string|null, integrationMode: boolean}}
 */
export function parseEmbedSurface(locationLike = window.location) {
  ...
  const integrationId = normalizeIdentifier(params.get('integration'))
  const integrationMode = embedded && !!surface && !!integrationId

  return Object.freeze({
    embedded,
    surface: integrationMode ? surface : null,
    integrationId: integrationMode ? integrationId : null,
