...
import { initSession, getSessionKey, setSessionKey, validateChatSessionCandidate } from '../session-manager.js?v=27'
import { initChat, activateSession, abortCurrentStream, getCurrentAgentInfo, focusChatInput, cancelChatInputFocusRetry } from '../chat-ui.js?v=28'
import { listSessions, deleteSession } from '../api-client.js?v=27'
...
import { EmbedContextController } from '../embed/context-controller.js?v=27'
...
const handleExternalChatSessionChange = (event) => {
  ...
  if (!mounted || !nextKey || nextKey === currentSessionKey) return
...
export async function mount(container) {
  ...
  if (floatingEnabled) {
    ...
    floatingToolbar.innerHTML = `
      <button id="floating-open-full" class="floating-assistant-control" type="button" aria-label="Open full assistant" title="Open full assistant">↗</button>
      <button id="floating-close" class="floating-assistant-control" type="button" aria-label="Close assistant" title="Close assistant">×</button>
    `
    ...
    embedContextController = new EmbedContextController({
      ...
      openButton: floatingToolbar.querySelector('#floating-open-full'),
